...
tableConfigs = spark.read.format("delta").table("mdf1.bronze.meta_dataloadingmetrics").filter("not IncrementalLoadFlag").collect()

for config in tableConfigs:

    print(config["DestinationTableName"])
