...
from pyspark.sql.types import StructType, LongType, StringType # pylint: disable=import-error # type: ignore
from pyspark.sql.functions import from_json, col, current_timestamp, to_date, from_utc_timestamp, lit # pylint: disable=import-error # type: ignore
...
class PipelineReaders:
    ...
    def read_dlt_cloud_files(spark, bronze_dataflow_spec, schema_json) -> DataFrame:
        ...
        schema_path = bronze_dataflow_spec.sourceDetails.get("schema_path", None)
        ...
        if "cloudFiles.useNotifications" in reader_config_options and reader_config_options["cloudFiles.useNotifications"] == "true":
            ...
            reader_config_options["cloudFiles.subscriptionId"] = cloudFileNotificationsConfig.get("cloudFiles.subscriptionId")
            reader_config_options["cloudFiles.resourceGroup"] = cloudFileNotificationsConfig.get("cloudFiles.resourceGroup")
            
            reader_config_options["cloudFiles.clientId"] = dbutils.secrets.get(
                cloudFileNotificationsConfig.get("secret_scope")
                ,cloudFileNotificationsConfig.get("cloudFiles.clientId"))
            
            reader_config_options["cloudFiles.clientSecret"] = dbutils.secrets.get(
                cloudFileNotificationsConfig.get("secret_scope")
                ,cloudFileNotificationsConfig.get("cloudFiles.clientSecret"))
            
            reader_config_options["cloudFiles.tenantId"] = cloudFileNotificationsConfig.get("cloudFiles.tenantId")            
        ...
        if schema_json and bronze_dataflow_spec.sourceFormat.lower() != "delta":
            ...
            source_schema_str = bronze_dataflow_spec.sourceDetails["source_schema"]

            if source_schema_str:
                source_schema_json = json.loads(source_schema_str)
                source_schema = StructType.fromJson(source_schema_json)
                # fix parquet file from GG to delta schema mismatch issue
                schema_str = bronze_dataflow_spec.sourceDetails['source_schema']
                source_schema_json = json.loads(schema_str)

                source_schema = StructType.fromJson(source_schema_json) 
                
                custom_parquet = bronze_dataflow_spec.sourceDetails.get("file_format", None) == "custom-parquet"
            
                if custom_parquet:
                    df = (
                        spark.readStream.format("cloudFiles")
                        .option("cloudFiles.format", "parquet")
                        .schema(source_schema)  # enforce schema to avoid schema inference issues
                        ...
                        .select(
                            ...
                            col("_metadata.file_modification_time").alias("file_load_datetime"),
                            to_date(col("_metadata.file_modification_time")).alias("batch_id"),
                            from_utc_timestamp(current_timestamp(), "America/New_York").alias("processing_time"),
                            col("op_type").alias("source_update_type") if "op_type" in [field.name for field in schema] else col(lit(None)).cast(StringType()).alias("source_update_type")
                        )
                    )            
                else:
                    df = (
            ...
            return (
                spark.readStream.format(bronze_dataflow_spec.sourceFormat) # to handle cloudFiles.parquet format
                .options(**reader_config_options)
                .load(source_path)
                .select(
                    "*",
                    ...
                    to_date(col("_metadata.file_modification_time")).alias("batch_id"),
                    from_utc_timestamp(current_timestamp(), "America/New_York").alias("processing_time"),
                    col("op_type").alias("source_update_type") if "op_type" in [field.name for field in schema_json['fields']] else col(lit(None)).cast(StringType()).alias("source_update_type")
                )
            )
