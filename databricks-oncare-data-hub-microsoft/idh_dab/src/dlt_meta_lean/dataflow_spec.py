...
class DataflowSpecUtils:
    ...
    @staticmethod
    def get_fhir_dataflow_spec(spark) -> List[FhirDataflowSpec]:
        """Get silver dataflow spec list."""
        DataflowSpecUtils.check_spark_dataflowpipeline_conf_params(spark, "fhir")

        dataflow_spec_rows = DataflowSpecUtils._get_dataflow_spec(spark, "fhir").collect()
        fhir_dataflow_spec_list: list[FhirDataflowSpec] = []
        for row in dataflow_spec_rows:
            fhir_dataflow_spec_list.append(FhirDataflowSpec(**row.asDict()))
        return fhir_dataflow_spec_list    
    ...
    def get_schema_json(spark, dataflow_spec):
        ...
        if reader_config_options["cloudFiles.format"].lower() == "parquet":
            schema = (
                ...
                .withColumn("source_file_path", lit(None).cast("STRING"))
                .withColumn("file_load_datetime", lit(None).cast("TIMESTAMP"))
                .withColumn("batch_id", lit(None).cast(DateType()))
                .withColumn("processing_time", lit(None).cast("TIMESTAMP"))
                .withColumn("source_update_type", lit(None).cast("STRING"))
                .limit(100)
            ).schema
