...
from pyspark.sql.functions import expr, col, struct, lit, from_json
from pyspark.sql.types import StructType, StructField, StringType, TimestampType, DateType
import inspect
import requests


from dlt_meta_lean.dataflow_spec import BronzeDataflowSpec, SilverDataflowSpec, FhirDataflowSpec, DataflowSpecUtils
from dlt_meta_lean.pipeline_readers import PipelineReaders

# Version 1: using function decorators
from cdc_function.silver_cdc import ChangeFunctionRegistry 
from cdc_function.fhir_cdc import FHIRChangeFunctionRegistry
from udf_function.fhir_udf import fhir_udf

# Version 2: using class methods
# from cdc_function.silver_cdc_v2 import ChangeFunctionRegistry
...
class DataflowPipeline:
    ...
    def __init__(self, spark, dataflow_spec, view_name, view_name_quarantine=None):
        ...
        if isinstance(dataflow_spec, BronzeDataflowSpec) or isinstance(dataflow_spec, SilverDataflowSpec) or isinstance(dataflow_spec, FhirDataflowSpec):
    ...
    def __initialize_dataflow_pipeline(self, spark, dataflow_spec, view_name, view_name_quarantine):
        ...
        if isinstance(dataflow_spec, SilverDataflowSpec) and not self.dataflowSpec.materializedView:
            if "idh_silver" in self.dataflowSpec.dataFlowGroup.lower():
                self.silver_schema = json.loads(dataflow_spec.schema)
            else:
                self.silver_schema = self.get_silver_schema()
        else:
            self.silver_schema = None

        if isinstance(dataflow_spec, FhirDataflowSpec) :
            self.fhir_schema = json.loads(dataflow_spec.schema)
            ...
            tenantId = "cddc1229-ac2a-4b97-b78a-0e5cacb5865c"
            fhir_base_scope = dataflow_spec.targetDetails("fhir_scope", None)
    ...
    def read(self):
        ...
        if isinstance(self.dataflowSpec, BronzeDataflowSpec):
            ...
            if not silverDataflowSpec.materializedView:  
                if "idh_silver" in self.dataflowSpec.dataFlowGroup.lower():
                    silver_view = f"vw_{silverDataflowSpec.targetDetails['table']}"
                    silver_change_view = f"vw_change_{silverDataflowSpec.targetDetails['table']}"

                    dlt.view(
                        self.read_silver,
                        name=silver_view,
                        comment=f"input dataset view for {silver_view}",
                    )

                    dlt.view(
                        self.read_silver_cdf,
                        name=silver_change_view,
                        comment=f"input dataset view for {silver_change_view}",
                    )                    
                else:  
                    dlt.view(
                        self.read_silver,
        ...
        elif isinstance(self.dataflowSpec, FhirDataflowSpec):
            fhirDataflowSpec: FhirDataflowSpec = self.dataflowSpec

            ## Create a view to read data

            fhir_view = f"vw_{fhirDataflowSpec.targetDetails['table']}"
            fhir_change_view = f"vw_change_{fhirDataflowSpec.targetDetails['table']}"
    ...
    def write_fhir(self):
        """Write silver tables."""
        fhir_dataflow_spec: FhirDataflowSpec = self.dataflowSpec

        self.udf_append_rows(self.access_token)
    ...
    def read_silver(self) -> DataFrame:
        ...
        ## If there no DQEs read from Bronze table
            ...
            if "idh_silver" in self.dataflowSpec.dataFlowGroup.lower():
                file_name = self.dataflowSpec.targetDetails["table"].lower()
                source_schema = self.dataflowSpec.sourceDetails["database"].split(".")[1]
                with open(f"./silver_queries/query_{file_name}.sql", 'r') as file:
                    query = file.read().replace('{bronze_database}', f'{source_schema}')
                raw_delta_table_stream = self.spark.sql(query)              
            else:
                raw_delta_table_stream = self.spark.readStream.table(
                    f"{source_database}.{source_table}"
                ).selectExpr(*select_exp) if self.uc_enabled else self.spark.readStream.load(
    ...
    def read_silver_cdf(self) -> DataFrame:
        silver_dataflow_spec: SilverDataflowSpec = self.dataflowSpec

        target_table = silver_dataflow_spec.targetDetails['table']
        target_database = silver_dataflow_spec.targetDetails['database']
        bronze_database = silver_dataflow_spec.sourceDetails["database"]
        silver_view = f"vw_{silver_dataflow_spec.targetDetails['table']}"

        # Version 1: using function decorators
        return ChangeFunctionRegistry.run(self.spark, target_table, bronze_database, target_database, silver_view)

        # Version 2: using class methods
        # registry = ChangeFunctionRegistry(self.spark, bronze_database, silver_view)
        # return registry.run(target_table)

    def read_fhir(self) -> DataFrame:
        """Read Silver tables."""
        fhir_dataflow_spec: FhirDataflowSpec = self.dataflowSpec
        source_database = fhir_dataflow_spec.sourceDetails["database"]
        source_table = fhir_dataflow_spec.sourceDetails["table"]
        select_exp = fhir_dataflow_spec.selectExp
        where_clause = fhir_dataflow_spec.whereClause

        ## If there are DQEs read from DQE view
        file_name = self.dataflowSpec.targetDetails["table"].lower()
        source_schema = self.dataflowSpec.sourceDetails["database"].split(".")[1]
        with open(f"./fhir_queries/query_{file_name}.sql", 'r') as file:
            query = file.read().replace('{silver_schema}', f'{source_schema}')
        raw_delta_table_stream = self.spark.sql(query)              
    ...
    def read_fhir_cdf(self) -> DataFrame:
        fhir_dataflow_spec: SilverDataflowSpec = self.dataflowSpec

        target_table = fhir_dataflow_spec.targetDetails['table']
        silver_database = fhir_dataflow_spec.sourceDetails["database"]
        fhir_view = f"vw_{fhir_dataflow_spec.targetDetails['table']}"

        # Version 1: using function decorators
        return FHIRChangeFunctionRegistry.run(self.spark, target_table, silver_database, fhir_view)
    ...
    def cdc_apply_changes(self):
        ...
        if isinstance(self.dataflowSpec, BronzeDataflowSpec):
            struct_schema = (
                StructType.fromJson(self.schema_json)
                if self.schema_json
                else StructType.fromJson(DataflowSpecUtils.get_schema_json(self.spark, self.dataflowSpec))
            )
        else:
            struct_schema = StructType.fromJson(self.silver_schema)

        if "idh_silver" not in self.dataflowSpec.dataFlowGroup.lower():
            reader_config_options = self.dataflowSpec.readerConfigOptions
            struct_schema.add(reader_config_options["cloudFiles.rescuedDataColumn"], StringType(), True)
            struct_schema.add("source_file_path", StringType(), True)
            struct_schema.add("file_load_datetime", TimestampType(), True)
            struct_schema.add("batch_id", DateType(), True)
            struct_schema.add("processing_time", TimestampType(), True)
            struct_schema.add("source_update_type", StringType(), True)
        ...
        siver_change_view = f"vw_change_{self.dataflowSpec.targetDetails['table']}"

        dlt.apply_changes(
            target=f"{self.dataflowSpec.targetDetails['table']}",
            source=siver_change_view if "idh_silver" in self.dataflowSpec.dataFlowGroup.lower() else self.view_name,
    ...
    def udf_append_rows(self, access_token):
        """Process record with udf for fhir."""

        struct_schema = StructType.fromJson(self.fhir_schema)
        ...
        def sync_fhir_api():
            fhir_change_view = f"vw_change_{self.dataflowSpec.targetDetails['table']}"

            cdc_df = dlt.readStream(fhir_change_view)

            update_udf = fhir_udf.get_udf(self.dataflowSpec.targetDetails['table'])        

            return (
                cdc_df
                .withColumn("fhir_sync_status", update_udf(col("id"), struct(col("*")), lit(access_token)))
    ...
    def invoke_dlt_pipeline(spark, layer):
        ...
        elif "fhir" == layer.lower():
            dataflowspec_list = DataflowSpecUtils.get_fhir_dataflow_spec(spark)
